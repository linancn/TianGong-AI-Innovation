...
def innovation_assessment(id: str) -> dict:
    ...
    with open(f"data/backdata/back_result_2.csv", mode="a+", newline="", encoding="utf-8") as file:
...
query = "WITH ranked_articels AS (SELECT row_number() OVER (ORDER BY uuid) as row_num, uuid FROM journals WHERE (embedding_time IS NOT NULL AND authors IS NOT NULL)) SELECT row_num, uuid FROM ranked_articels WHERE row_num BETWEEN 200001 AND 300000"
